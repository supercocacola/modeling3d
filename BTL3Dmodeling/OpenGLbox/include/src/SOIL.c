...
int query_NPOT_capability( void )
{
	...
	if( has_NPOT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		if(!check_GL_extension("GL_ARB_texture_non_power_of_two" ))
		{
...
int query_tex_rectangle_capability( void )
{
	...
	if( has_tex_rectangle_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		&&
			(!check_GL_extension("GL_EXT_texture_rectangle" ))
		&&
			(!check_GL_extension("GL_NV_texture_rectangle" ))
			)
		{
			/*	not there, flag the failure	*/
...
int query_cubemap_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if((!check_GL_extension("GL_ARB_texture_cube_map" ))
		&& (!check_GL_extension("GL_EXT_texture_cube_map" ))
			)
		{
			/*	not there, flag the failure	*/
...
int query_DXT_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if( !check_GL_extension("GL_EXT_texture_compression_s3tc" ) )
		{
			/*	not there, flag the failure	*/
...
// Check if the current graphics driver supports an OpenGL extension.
// Added by Ying Zhu (Georgia State University) in October 2014
// Use this function to check the availability of OpenGL extensions 
// instead of using glGetString(GL_EXTENSIONS), which was removed from
// OpenGL 3.1. The function call glGetString(GL_EXTENSIONS) may cause errors 
// on some computers. 
unsigned int check_GL_extension(const char *string_extension) {
	
	GLint n, i;
	glGetIntegerv(GL_NUM_EXTENSIONS, &n);
	for (i = 0; i < n; i++) {
		if (NULL != strstr((const char *) glGetStringi(GL_EXTENSIONS, i), string_extension)) {
			// If the current driver support this extension, return 1. 
			return 1;
		}
	}

	// // If the current driver support this extension, return 0.
	return 0;
}